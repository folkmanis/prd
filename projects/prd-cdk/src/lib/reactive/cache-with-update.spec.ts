...
const update: Data = { _id: 1, d: 'c' };

const insertion: Data = { _id: 3, d: 'e' };
...
describe('Cache with update stream', () => {
  ...
  it('should pass initial data without changes', () => {
    testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      expectObservable(
        init.pipe(cacheWithUpdate(of() as Observable<Data>, compareFn)),
  ...
  it('should change data', () => {
    testScheduler.run(
      ({ cold, hot, expectObservable, expectSubscriptions }) => {
        ...
        expectObservable(
          init.pipe(cacheWithUpdate(upd, compareFn)),
          unsub,
        ).toBe(expected, { x: initial, y: result, z: inserted });
        expectSubscriptions(init.subscriptions).toBe(subs);
